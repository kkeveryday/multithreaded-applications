...
LPTHREAD_START_ROUTINE FileProc(struct str* f)
{
	...
	if (!fin) {
		cout << "Файл " << (*f).fname << " не открыт" << endl; //вывод сообщения, если файл не был открыт и выход из программы
		exit(0);
	}
	int n = 0;
...

{
	...
	for (i = 0; i < N; i++)
	{
		//Вызов функции обработки файла с последующим выводом полученных значений
		FileProc(&str[i]);
	}
